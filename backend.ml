...
(* Puts the address computed by a gep computation into %rax. On 64-bit x86,
   the getelementptr instruction supports only i64 indices. Moreover, the
   first index must be exactly 0l. Subsequent indices are interpreted as
   offsets whose size is determined by the type of the op pointer.

   Full LLVM allows non-constant indexing into structured types, but here we
   only  allow non-constant indices into arrays, whose element sizes are known
   statically *)
...
let compile_insn ctxt (uid, i) : X86.ins list =
  ...
  | Icmp (cnd, _, op1, op2) -> (op_to_rax op1) ::
                               (op_to_rcx op2) :: Asm.([ Cmpq, [~%Rcx; ~%Rax]
...
let uid_layout args (block, lbled_blocks) : layout =
...
let compile_fdecl tdecls name { fty; param; cfg } =
  ...
  let layout = uid_layout param cfg in
  let init_arg_code =
