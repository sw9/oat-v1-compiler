...
let opt_ll  = ref false
let opt_x86 = ref false    
...
let process_ll_ast path file ll_ast =
  ...
  let _ =
    if !clang then begin
      ...
      if !print_x86_flag then Platform.sh (Printf.sprintf "cat %s" dot_s_file) Platform.raise_error
    end else begin
...
let string_of_ll_ast path ll_ast =
  let ll_str = Ll.string_of_prog ll_ast in
  let triple = if !linux then !linux_target_triple else "x86_64-apple-macosx10.10.0" in
  let prog = Printf.sprintf "; generated from: %s\ntarget triple = \"%s\"\n%s\n%s\n"
      path triple Frontend.oat_ll_prelude ll_str in
  prog
...
let process_oat_ast path file oat_ast =
  ...
  let _ = Platform.verb @@ Printf.sprintf "writing file: %s\n" dot_ll_file in
  let prog = string_of_ll_ast path ll_ast in
  let _ = write_file dot_ll_file prog in
